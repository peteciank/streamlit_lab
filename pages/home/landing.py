...
# 1) Page configuration: wide layout & show the sidebar toggle
st.set_page_config(
    page_title="RowTok",
    layout="wide",
...
# 2) Inject CSS (we remove only the 'header {visibility:hidden;}' line so the toggle stays visible)
st.markdown("""
<style>
    ...
    .header-container {
        ...
        margin-bottom: 1rem; min-height: 100px;
    }
    .rowtok-logo {
    ...
    .finger-pointer {
        position: fixed !important;
        top: 25px !important; left: 35px !important;
    ...
    @keyframes fingerBounce {
        0%,20%,50%,80%,100% { transform: translateY(0px) rotate(-15deg); }
    ...
    .contact-bubble {
        position: fixed !important;
        bottom: 140px !important; right: 25px !important;
    ...
    .contact-button {
        ...
        background: #25D366 !important; border: none !important;
    ...
    .contact-menu {
        ...
        bottom: 75px !important; right: 0 !important;
        width: 260px !important; background: white !important;
    ...
    @media (prefers-color-scheme: dark) {
        .section-title { color: #74b9ff !important; }
    }
    .section-text {
        font-size: 1.1rem; line-height: 1.6; color: #333; text-align: center;
    }
    ...
    @media (max-width: 768px) {
        .rowtok-title { font-size: 2.8rem; }
        .rowtok-logo  { width: 60px; height: 60px; }
        .header-container { gap: 20px; }
        .content-section  { margin: 1rem; padding: 1.5rem; }
        .features-grid    { grid-template-columns: 1fr; }
        .finger-pointer   { left: 30px !important; font-size: 1.8rem !important; top: 20px !important; }
        .contact-bubble   { bottom: 120px !important; right: 15px !important; }
        .contact-menu     { width: 240px !important; }
    }
</style>
""", unsafe_allow_html=True)

# 3) Load your logo (or fallback emoji)
def get_logo_path():
    return Path(__file__).parent.parent / 'static' / 'images' / 'logo.png'
...
# 4) Hero + Contact bubble + JS via components.html
hero_html = f"""
...
(function() {{
  ...
  btn?.addEventListener('click', e => {{
    ...
    btn.style.transform = open ? 'scale(1.1)' : 'scale(1)';
  }});
